...
import com.example.goev.database.UserRepository
...
class ForumViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val userRecords = listOf<UserData>(
        UserData("user1", "user1"),
        UserData("user2", "user2"),
        UserData("user3", "user3"),
        UserData("user4", "user4"),
        UserData("user5", "user5")
    )
    ...
    val forumCommentRecords = listOf<ForumCommentData>(
        ...
        ForumCommentData(
            3, 1, "user2", "That's great to hear! Thanks a lot!", LocalDateTime.parse(
                "2023-05-14T16:21:25", DateTimeFormatter.ISO_DATE_TIME
        ...
        ForumCommentData(
            9, 3, "user1", "I agree, the Nissan Leaf is a great option. ", LocalDateTime.parse(
                "2023-05-14T16:21:25", DateTimeFormatter.ISO_DATE_TIME
            ).toInstant(java.time.ZoneOffset.UTC).toEpochMilli(), LocalDateTime.parse(
                "2023-05-14T16:21:25", DateTimeFormatter.ISO_DATE_TIME
            ).toInstant(java.time.ZoneOffset.UTC).toEpochMilli()
        ...
        ForumCommentData(
            15, 5, "user4", "Alright, thanks.", LocalDateTime.parse(
                "2023-05-14T16:21:25", DateTimeFormatter.ISO_DATE_TIME
            ).toInstant(java.time.ZoneOffset.UTC).toEpochMilli(), LocalDateTime.parse(
                "2023-05-14T16:21:25", DateTimeFormatter.ISO_DATE_TIME
            ).toInstant(java.time.ZoneOffset.UTC).toEpochMilli()
    ...
    fun loadCommentList(postId: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val forumCommentList = postCommentList?.map { comment ->
                ...
                var respondedCommentData = ForumCommentData(0, 0, "", "", 0, 0)
                if (isReply) {
    ...
    fun loadSearchList(search: String){
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val searchResult = forumDataList?.filter { it.forumPostData.title.contains(search,ignoreCase = true) == true}

            withContext(Dispatchers.Main) {
                _searchDataList.postValue(searchResult)
                if(searchResult!!.isEmpty()){
                    val toast = Toast.makeText(getApplication(), "No Search Result",Toast.LENGTH_SHORT)
                    toast.show()
                }
            }
        }
    ...
    fun addforumReplies(initialCommentId: Int, parentCommentId: Int, postId: Int, content: String) {
        viewModelScope.launch(Dispatchers.IO) {
            var forumCommentData = ForumCommentData(0, 0, "", "", 0, 0)
    ...
    fun addPost(title : String, content: String) {
        ...
        fun insertSampleData() {
            ...
            viewModelScope.launch(Dispatchers.IO) {
                if (forumPostRepository.getAllPost()?.count() ?: 0 != 0) {
                } else {
                    ...
                    for (hashtag in forumHashtagRecords) {
                    ...
                    for (postHashtag in forumPostHashtagRecords) {
